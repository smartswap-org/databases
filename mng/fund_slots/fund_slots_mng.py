...
def get_last_fund_slots(cursor, bot_name):
    try:
        ...
        logger.error(f"Error executing get_last_fund_slots: {e}")
        return None

def init_fund_slots(cursor, bot_name, funds):
    if get_last_fund_slots(cursor, bot_name): 
        logger.warning(f"Bot {bot_name} already has funds in the database.")
        return
...
def insert_fund_slots(cursor, bot_name, funds):
    try:
        cursor.execute('''
            ...
            VALUES (?, ?)
        ''', (bot_name, funds))
        logger.info(f"Inserted funds for bot {bot_name}.")
    except Exception as e:
        logger.error(f"Error executing insert_fund_slots: {e}")
