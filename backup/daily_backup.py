...
# Create a new logger instance with appropriate settings
log = Logger(
        ...
        for date_str, folders in backup_by_date.items():
            if len(folders) > 1:
                ...
                for folder in sorted_folders[1:]:
                    log.info(f"Removing duplicate backup: {folder.name}")
                    for file in folder.glob('*'):
                        file.unlink()
                    folder.rmdir()
                
    except Exception as e:
        log.error(f"Error while cleaning duplicate backups: {str(e)}")
...
def check_today_backup():
    ...
    try:
        ...
        log.warning(f"No backup found for today ({today_str})")
        return False
        
    except Exception as e:
        log.error(f"Error while checking backups: {str(e)}")
        return False
...
def main():
    ...
    while True:
        try:
            ...
            if not has_backup:
                log.info("Starting backup process since no backup was found.")
                for db in db_names: 
                    backup_mysql(db)
            ...
            log.error(f"Error in main monitoring loop: {str(e)}")
            time.sleep(3600)
